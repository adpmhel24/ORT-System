...
class ListOfBps extends StatefulWidget {
  const ListOfBps({super.key, this.onSelection});

  final Function(BusinessPartnerModel)? onSelection;

  @override
  State<ListOfBps> createState() => _ListOfBpsState();
}

class _ListOfBpsState extends State<ListOfBps> {
  static final _tblColumns = BusinessPartnerModel.headers
      .map(
        (e) => ColumnModel(
  ...
  Widget build(BuildContext context) {
    return BlocSelector<BpBloc, BpState, List<BusinessPartnerModel>>(
      selector: (state) => state is BpsLoadedState ? state.datas : [],
